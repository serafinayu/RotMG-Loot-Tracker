...
const ItemGrid = () => {
  ...
  const renderItems = (itemsArray, groupName) => {
    return (
      <div>
        <div className="bg-[#1A1A1A] text-white w-full text-center p-1.5 mb-2 rounded-md select-none">
          {groupName}
        </div>
        <div className="flex flex-row flex-wrap gap-2 px-5 mt-4 mb-8">
          {itemsArray.map(([itemId, item], index) => (
            <div
              ...
              //
              onClick={() => {
                !showCollections && selectedItem === itemId
                  ? setSelectedItem("")
            ...
            >
              ...
              <p
                className="absolute right-1 bottom-[0.5px] text-white font-bold text-shadow-outline"
                style={{
                  textShadow:
  ...
  return (
    <div className="item-grid-scrollbar my-5 px-3 h-[50vh] md:h-[70vh]">
      <div className="">
        {/* Tiered Items */}
